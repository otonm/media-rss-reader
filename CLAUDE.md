...
│       ├── controls.js       # Button wiring (autoscroll, mute, show-seen, status modal)
│       └── style.css         # Layout, CSS variables for theming, transitions, modal
│
└── tests/
...
| `REDDIT_FEEDS_API_URL` | `http://127.0.0.1:9090` | URL of the Reddit Feeds status API |
...
GET  /api/reddit-feeds/status      → proxies GET /status from Reddit Feeds API
                                      (configured via REDDIT_FEEDS_API_URL)
                                      → {feeds: [{name, last_status, last_fetch,
                                        last_item_count, total_items}], last_run}
```
